...
void reset(SDL_Rect &destinoPeixe, int& peixeMov, int& score, int& comidaRandX, int& coralMov, int& linhaMOV, int& comidaMov, int& TAMcoral) {
    peixeMov = 0;
    score = 0;
    destinoPeixe.w = 100;
    destinoPeixe.h = 100;
    ...
    if (destinoPeixe.y + destinoPeixe.h > destinoCoral.y + 10) {
        if (destinoPeixe.x + destinoPeixe.w > destinoCoral.x + 5) {
            if (destinoPeixe.x < destinoCoral.x + destinoCoral.w) {
                reset(destinoPeixe, peixeMov, score, comidaRandX, coralMov, linhaMOV, comidaMov, TAMcoral);
            }
        }
    }
};
...
void HitBoxAnzol(SDL_Rect destinoAnzol, SDL_Rect &destinoPeixe, int& peixeMov, int& score, int& comidaRandX, int& coralMov, int& linhaMOV, int& comidaMov, int& TAMcoral) {
